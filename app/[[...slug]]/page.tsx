...
import ProvenanceSimulatorPage from "@/app/provenance/simulator/page";
...
export default async function RootRoute({ params, searchParams }: { params: Promise<{ slug?: string[] }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const [{ slug }] = await Promise.all([params, searchParams]); const parts = slug ?? []; const props = { searchParams };
  if (parts.length === 0) return <MarketingHome />;
  switch (parts[0]) {
    case "awards": return <AwardsPage {...(props as any)} />; case "buyers": return <BuyersPage {...(props as any)} />; case "cases": return <CasesPage />;
    case "companies": if (parts.length === 2) return <CompanyDetailPage params={Promise.resolve({ companyId: parts[1] })} searchParams={searchParams} />; return <CompaniesPage {...(props as any)} />;
    case "context": return <ContextPage {...(props as any)} />; case "demo": return <DemoPage />; case "field": return <FieldPage searchParams={searchParams as Promise<{ tender?: string; requirement?: string }>} />;
    case "graph": return <GraphPage {...(props as any)} />; case "investigate": return <InvestigatePage {...(props as any)} />; case "investigations": return <InvestigationsPage {...(props as any)} />;
    case "map": return <MapPage {...(props as any)} />; case "monitoring": return <MonitoringPage />; case "profile": return <ProfilePage {...(props as any)} />;
    case "provenance": if (parts[1] === "simulator") return <ProvenanceSimulatorPage />; return <ProvenancePage />;
    case "provenance-simulator": return <ProvenanceSimulatorPage />;
