...
def _indian_tender_where() -> object:
    return indian_source_clause(Tender.source_name)
...
def list_awards(
    ...
    filters = [_indian_tender_where(), Tender.deleted_at.is_(None)]
    if q:
        term = f"%{q.strip()}%"
        filters.append(or_(Company.name.ilike(term), Tender.title.ilike(term), Tender.reference_number.ilike(term)))
    ...
    awards = db.execute(_apply_award_sort(statement, sort).limit(limit).offset(offset)).unique().scalars().all()
...
def get_overview(db: Session = Depends(get_db)) -> OverviewResponse:
    indian_filter = _indian_tender_where()
    tender_filter = [indian_filter, Tender.deleted_at.is_(None)]
    award_filter = [indian_filter, Tender.deleted_at.is_(None)]

    total_tenders = db.scalar(select(func.count()).select_from(Tender).where(*tender_filter)) or 0
    total_companies = db.scalar(
        select(func.count(func.distinct(Award.company_id)))
        .select_from(Award).join(Tender, Award.tender_id == Tender.id)
        .where(*award_filter, Award.company_id.is_not(None))
    ) or 0
    total_awards = db.scalar(select(func.count()).select_from(Award).join(Tender, Award.tender_id == Tender.id).where(*award_filter)) or 0
    total_tender_value = db.scalar(select(func.coalesce(func.sum(Tender.estimated_value), 0)).select_from(Tender).where(*tender_filter)) or 0
    total_awarded_value = db.scalar(select(func.coalesce(func.sum(Award.award_value), 0)).select_from(Award).join(Tender, Award.tender_id == Tender.id).where(*award_filter)) or 0
    average_tender_value = db.scalar(
        select(func.coalesce(func.avg(Tender.estimated_value), 0)).select_from(Tender).where(*tender_filter, Tender.estimated_value.is_not(None))
    ) or 0
    buyers = db.scalar(select(func.count(func.distinct(Tender.procuring_entity))).select_from(Tender).where(*tender_filter)) or 0

    # Bid-level records are not present in the current Indian dataset. A single
    # recorded awardee is an award fact, not proof that exactly one bidder
    ...
    top_suppliers = [
        ...
        for row in db.execute(
            select(Company.id, Company.name, func.count(Award.id), supplier_value)
            .select_from(Award)
            .join(Company, Award.company_id == Company.id)
            .join(Tender, Award.tender_id == Tender.id)
            .where(*award_filter)
    ...
    monthly_rows = db.execute(
        select(month_expr, func.count(Tender.id), func.coalesce(func.sum(Tender.estimated_value), 0))
        .where(*tender_filter, Tender.published_date.is_not(None))
        .group_by(month_expr)
        .order_by(month_expr.desc())
        .limit(12)
    ).all()
    monthly = [MonthlyPoint(month=row[0], tenders=int(row[1]), value=row[2]) for row in reversed(monthly_rows)]
    ...
    return OverviewResponse(totals=totals, top_buyers=top_buyers, top_suppliers=top_suppliers, monthly=monthly, sources=sources)
...
def get_timeline(limit: int = Query(default=60, ge=1, le=200), db: Session = Depends(get_db)) -> TimelineResponse:
    events: list[TimelineEvent] = []
    tender_filter = [_indian_tender_where(), Tender.deleted_at.is_(None)]

    published_tenders = db.scalars(
        select(Tender).where(*tender_filter, Tender.published_date.is_not(None)).order_by(Tender.published_date.desc()).limit(limit)
    ).all()
    for tender in published_tenders:
        events.append(TimelineEvent(date=tender.published_date, kind="tender_published", title=tender.title, subtitle=tender.procuring_entity, reference=tender.reference_number, entity_type="tender", entity_id=tender.id))

    closing_tenders = db.scalars(
        select(Tender).where(*tender_filter, Tender.closing_date.is_not(None)).order_by(Tender.closing_date.desc()).limit(limit)
    ).all()
    for tender in closing_tenders:
        events.append(TimelineEvent(date=tender.closing_date, kind="tender_closing", title=tender.title, subtitle=tender.procuring_entity, reference=tender.reference_number, entity_type="tender", entity_id=tender.id))

    awards = db.execute(
        select(Award)
        .join(Tender, Award.tender_id == Tender.id)
        .where(*tender_filter, Award.award_date.is_not(None))
        .options(joinedload(Award.company), joinedload(Award.tender))
    ...
    for award in awards:
        ...
        events.append(TimelineEvent(date=award.award_date, kind="award", title=award.company.name, subtitle=f"Awarded: {tender_title}", reference=award.tender.reference_number if award.tender is not None else None, entity_type="company", entity_id=award.company.id))
...
def get_geography(db: Session = Depends(get_db)) -> GeographyResponse:
    ...
    award_counts: dict[UUID, int] = {
        row[0]: int(row[1])
        for row in db.execute(
            select(Award.tender_id, func.count(Award.id)).join(Tender, Award.tender_id == Tender.id).where(*tender_filter).group_by(Award.tender_id)
        ).all()
    }

    tenders = db.execute(
        select(Tender.id, Tender.title, Tender.procuring_entity, Tender.estimated_value).where(*tender_filter)
    ).all()
    ...
    total = len(tenders)
    regions = [Region(region=region, tenders=count, value=value_totals[region], awards=award_totals[region]) for region, count in tender_counts.items()]
    regions.sort(key=lambda item: item.tenders, reverse=True)
    return GeographyResponse(regions=regions, matched=matched, unmatched=total - matched, total=total)
...
def _apply_award_sort(statement: Select[tuple[Award]], sort: str) -> Select[tuple[Award]]:
    ...
    if sort == "buyer":
        return statement.order_by(Tender.procuring_entity.asc().nullslast(), Award.created_at.desc(), Award.id.desc())
