...
from app.connectors.world_bank import WorldBankProcurementImporter  # noqa: E402
from app.db.session import SessionLocal  # noqa: E402
from app.models import Award, Company, Tender  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import raw World Bank procurement notice JSON files.")
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=ROOT_DIR / "data" / "raw" / "world_bank",
        help="Directory containing raw World Bank notice JSON files.",
    )
...
def main() -> int:
    ...
    with SessionLocal() as session:
        stats = WorldBankProcurementImporter(session=session, batch_size=args.batch_size).import_directory(args.input_dir)
