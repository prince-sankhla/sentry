...
app = FastAPI(title="SENTRY FIELD Local Gateway", version="0.4.0")
app.add_middleware(
    CORSMiddleware,
...
_lock = Lock()
_state = {
    "running": False, "camera_url": DEFAULT_CAMERA_URL, "fps": 0.0, "inference_ms": 0.0,
    ...
    "capabilities": [], "confidence": DEFAULT_CONFIG.confidence,
...
def _load_demo_tenders() -> list[dict]:
    try:
        return json.loads(DEMO_TENDER_FILE.read_text(encoding="utf-8"))
...
def tender(tender_key: str):
    items = _load_demo_tenders()
    found = next((x for x in items if x.get("id") == tender_key or x.get("tender_id") == tender_key), None)
    if not found:
...
def _stream(camera_url: str, confidence: float, every_n_frames: int, mission_id: str | None,
            requirement_id: str | None, capabilities: list[str]) -> Generator[bytes, None, None]:
    config = build_config(
        source=camera_url,
    ...
    _set(
        running=True, camera_url=camera_url, last_error=None,
        mission_id=mission_id, requirement_id=requirement_id,
        capabilities=capabilities, confidence=config.confidence,
    ...
    try:
        while True:
            ...
            for d in detections:
                ...
                cv2.putText(display, f"{d.label} {d.confidence:.2f}", (x1, max(22, y1 - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 0, 0), 2)
                _events.appendleft({
                    "type": d.label, "confidence": round(d.confidence, 3), "bbox": d.bbox,
                    "detector": d.detector, "track_id": d.track_id, "observed_at": time.time(),
                    "mission_id": mission_id, "requirement_id": requirement_id,
                })
            ...
            if ids:
                _events.appendleft({
                    "type": "identity", "value": " | ".join(ids)[:200],
                    "detector": "qr/barcode/ocr", "observed_at": time.time(),
                    "mission_id": mission_id, "requirement_id": requirement_id,
                })
            now = time.monotonic()
            dt = now - prev
            ...
            with _lock:
                _state.update({
                    "fps": round(fps, 1), "inference_ms": round(ms, 1),
                    "findings": len(detections), "evidence": len(_events),
                    "last_detection": {
                        "type": detections[0].label, "confidence": round(detections[0].confidence, 3),
            ...
            cv2.putText(display, f"SENTRY FIELD | FPS {fps:.1f} | {ms:.0f}ms | findings {len(detections)}",
                        (14, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 2)
            if ids:
                cv2.putText(display, f"ID: {' | '.join(ids)[:110]}", (14, 70),
...
def stream(
    ...
    return StreamingResponse(
        _stream(
            _camera(camera_url), confidence, every_n_frames, mission_id, requirement_id,
            _caps(capabilities.split(',') if capabilities else None),
        ),
