...
def _stream(camera_url:str,confidence:float,every_n_frames:int,mission_id:str|None,requirement_id:str|None,capabilities:list[str])->Generator[bytes,None,None]:
    ...
    scanner=FieldScanner(config);cap=cv2.VideoCapture(camera_url);cap.set(cv2.CAP_PROP_BUFFERSIZE,1)
    ...
    _events.clear();_set(running=True,camera_url=camera_url,last_error=None,mission_id=mission_id,requirement_id=requirement_id,capabilities=capabilities,confidence=config.confidence,every_n_frames=config.every_n_frames)
    ...
    try:
        while True:
            ...
            idx+=1;detections,context,qr,barcode,ocr,_=scanner.scan(frame,idx);display=frame.copy()
            for d in detections:
    ...
    finally:cap.release();_set(running=False)
