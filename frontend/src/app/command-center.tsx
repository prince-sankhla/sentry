...
export function CommandCenter() {
  ...
  if (failed) return <div className="mt-6"><ErrorState message="SENTRY could not load the command center. Check the data connection and retry." /></div>;
  ...
  return (
    <div className="mt-2 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        <KpiCard label="High-severity reviews" value={formatNumber(risk.summary.high)} tone="danger" icon={<ShieldAlert className="h-4 w-4" />} spark={spark(5)} />
        <KpiCard label="Bid participation" value={t.single_bidder_tenders > 0 ? formatNumber(t.single_bidder_tenders) : "N/A"} tone="info" icon={<Radar className="h-4 w-4" />} spark={spark(4)} />
      </div>
      ...
      <Reveal className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <Section eyebrow="Concentration" title="Top buyers by awarded value" action={<Link href="/reports" className="text-xs text-accent hover:underline">View reports →</Link>}>
          {topBuyers.length === 0 ? <EmptyState message="No buyer records are available." /> : (
            <HBarChart
        ...
        <Section eyebrow="Suppliers" title="Top suppliers by awarded value" action={<Link href="/companies" className="text-xs text-accent hover:underline">View suppliers →</Link>}>
          {topSuppliers.length === 0 ? <EmptyState message="No supplier records are available." /> : (
      ...
      <Reveal className="grid grid-cols-1 gap-5 lg:grid-cols-[1.6fr_1fr]">
        <Section eyebrow="Trends" title="Procurement value trend" action={<Link href="/reports" className="text-xs text-accent hover:underline">Analytics →</Link>}>
          {trend.length === 0 ? <EmptyState message="No dated procurement records are available to chart." /> : (
            <AreaTrend categories={trend.map((m) => m.month.slice(2))} values={trend.map((m) => Number(m.value) || 0)} color={CHART.accent} height={220} valueFormatter={(v) => formatCompactMoney(String(v))} />
          )}
        </Section>

        <Section eyebrow="Investigation engine" title="Reasoning status" action={<Link href="/investigations" className="text-xs text-accent hover:underline">Open workspace →</Link>}>
          <AiStatus />
        </Section>
      </Reveal>
      ...
      <Reveal className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <Section eyebrow="Records" title="Recent tender records" action={<Link href="/tenders" className="text-xs text-accent hover:underline">View all →</Link>}>
                  ...
                  <Link href={`/tenders/${tender.id}`} className="flex items-center justify-between gap-4 py-2.5 transition hover:opacity-90">
                    <span className="flex min-w-0 items-center gap-3">
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block truncate text-xs text-faint">{tender.procuring_entity ?? "Procuring entity not available"} · {formatDate(tender.published_date)}</span>
                      </span>
                    </span>
