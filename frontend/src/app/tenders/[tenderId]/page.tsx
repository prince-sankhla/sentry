...
type PageProps = { params: Promise<{ tenderId: string }> };
                ...
                {tender.awards.map((award) => (
                  <div className="grid gap-3 rounded-2xl border border-border bg-bg-2 p-4 transition hover:bg-surface-2 md:grid-cols-[1fr_auto]" key={award.id}>
                    ...
                    <div className="flex items-center gap-3 text-left md:text-right">
                      <div>
                        <div className="text-sm font-semibold tabular-nums text-text">{formatMoney(award.award_value, award.currency)} {award.currency}</div>
                        <div className="mt-1 text-xs text-muted">{formatDate(award.award_date)}</div>
                      </div>
                      <InvestigateAction query={award.company.name} label="Investigate supplier" size="sm" variant="subtle" />
                    </div>
                  </div>
                ))}
              </div>
...
function IntelligenceSignals({ signals }: { signals: ProcurementIntelligenceSignal[] }) {
  ...
  return <div className="grid gap-3">{signals.map((signal) => (
    <div className="rounded-2xl border border-border bg-bg-2 p-4" key={`${signal.type}-${signal.company_id}-${signal.buyer}`}>
      ...
      {signal.evidence.length > 0 ? <div className="mt-3 flex flex-wrap gap-1.5">{signal.evidence.map((item, index) => <span className="rounded-md border border-border bg-bg-2 px-2 py-1 text-[11px] text-muted" key={`${item}-${index}`}>{item}</span>)}</div> : null}
    </div>
  ))}</div>;
}
...
function Detail({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-border bg-bg-2 p-3"><dt className="text-xs font-semibold uppercase tracking-[0.08em] text-faint">{label}</dt><dd className="mt-1 text-sm font-semibold text-text">{value}</dd></div>; }
