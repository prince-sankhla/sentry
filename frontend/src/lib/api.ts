...
export function getTenders({ limit = 25, offset = 0, q, sort = "newest" }: TenderListParams = {}): Promise<TenderListResponse> {
...
export function getPriorityQueue(limit = 8): Promise<PriorityQueueResponse> { const params = new URLSearchParams({ limit: String(limit) }); return apiGet<PriorityQueueResponse>(`/api/investigations/priority-queue?${params.toString()}`); }
...
export function getCompanyAwards(companyId: string, limit = 25, offset = 0): Promise<CompanyAwardHistoryResponse> { const params = new URLSearchParams({ limit: String(limit), offset: String(offset) }); return apiGet<CompanyAwardHistoryResponse>(`/api/companies/${companyId}/awards?${params.toString()}`); }
...
export function getProcurementEvidence(query: string, limit = 25): Promise<ProcurementEvidenceResponse> { const params = new URLSearchParams({ q: query, limit: String(limit) }); return apiGet<ProcurementEvidenceResponse>(`/api/web/procurement-evidence?${params.toString()}`); }
...
export function globalSearch(query: string, limit = 10): Promise<SearchResponse> { const params = new URLSearchParams({ q: query, limit: String(limit) }); return apiGet<SearchResponse>(`/api/search?${params.toString()}`); }
export function autocomplete(query: string, limit = 10): Promise<AutocompleteResponse> { const params = new URLSearchParams({ q: query, limit: String(limit) }); return apiGet<AutocompleteResponse>(`/api/search/autocomplete?${params.toString()}`); }
...
export function getBuyerProfile(name: string): Promise<ProfileResponse> { const params = new URLSearchParams({ name }); return apiGet<ProfileResponse>(`/api/profiles/buyer?${params.toString()}`); }
