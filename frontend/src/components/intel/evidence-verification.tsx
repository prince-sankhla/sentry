...
function saveReviews(key: string, value: Record<string, ReviewState>) {
  try {
...
export function EvidenceVerification({
  ...
  data: InvestigationPackage | null;
  initialQuery?: string;
}) {
  ...
  return (
    <Section
      ...
      title="Evidence verification & reasoning"
      action={
        <Button variant="ghost" size="sm" onClick={resetReviews}>
          <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> Reset
        </Button>
      }
    >
      <div className="mb-4 rounded-xl border border-border bg-bg-2/40 p-3 text-[11px] leading-relaxed text-muted">
        Review labels are analyst assessments. They do not change deterministic risk calculations.
      </div>
