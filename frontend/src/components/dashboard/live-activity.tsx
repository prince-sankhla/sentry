...
import { Award, FileText, Flag, Building2 } from "lucide-react";
    ...
    <ul className="space-y-1">
      {items.map((item, i) => {
        ...
        const body = (
          ...
          >
            <span className={`mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-md border border-border bg-bg-2 ${item.tone}`}>
            ...
            <span className="shrink-0 text-[11px] text-faint">{item.when}</span>
          </motion.div>
        );
