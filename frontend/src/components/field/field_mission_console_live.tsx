...
export function FieldMissionConsoleLive({
  tenderKey,
  requirementId,
}: {
  tenderKey?: string;
  ...
  const evidenceEvents = useMemo(
    () => {
      ...
      return events.filter((event) => {
        if (event.type !== "evidence" || !event.frame_url) return false;
        const key = `${event.capability || "evidence"}:${event.track_id || event.frame_url}`;
  ...
  const uniqueDetections = useMemo(() => {
    ...
    for (const event of detectionEvents) {
      keys.add(`${event.capability || "detection"}:${event.track_id || event.observation || "unknown"}`);
    }
    return keys.size;
  }, [detectionEvents]);
    ...
    async function load() {
      try {
        ...
        const planResponse = await fetch(
          `${BACKEND}/api/investigations/tenders/${encodeURIComponent(tenderKey)}/field-verification`,
          { cache: "no-store" },
        );
        ...
        const statusResponse = await fetch(
          `${BACKEND}/api/investigations/tenders/${encodeURIComponent(plan.id)}/field-verification-status`,
          { cache: "no-store" },
        );
  ...
  async function submitVerification() {
    ...
    const seen = new Set<string>();
    const observations = evidenceEvents
      .filter((event) => {
        const key = `${event.capability || "evidence"}:${event.track_id || event.frame_url}`;
        if (seen.has(key)) return false;
        seen.add(key);
    ...
    <main className="mx-auto w-full max-w-[1900px] space-y-5">
      <section className="rounded-2xl border border-accent/20 bg-surface p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.18em] text-accent">
              <Radar className="h-3.5 w-3.5" /> SENTRY FIELD / VERIFICATION RECORD
            </div>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-text">
              {completed ? "Update physical verification" : "Physical verification"}
            </h1>
            <p className="mt-2 max-w-3xl text-sm text-muted">
              {completed
                ? `Creating verification version ${(verification?.latest?.version || 1) + 1}; the previous record remains immutable.`
                : "The first completed submission becomes immutable verification version 1."}
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <Badge tone={completed ? "success" : "muted"}>
              {completed ? `Verified v${verification?.latest?.version}` : "Not yet verified"}
            </Badge>
            <Badge tone={online ? "success" : "muted"}>
              <Wifi className="mr-1 inline h-3 w-3" />
              {online ? "Gateway online" : "Gateway offline"}
            </Badge>
            <Badge tone={gpsLive ? "success" : "muted"}>
              <Navigation className="mr-1 inline h-3 w-3" />
              {gpsLive ? "GPS live" : gpsWatching ? "GPS waiting" : "GPS idle"}
            </Badge>
            {live ? (
              <button
                type="button"
                onClick={() => void stopMission()}
                className="rounded-xl border border-danger/30 bg-danger/10 px-4 py-2.5 text-xs font-semibold text-danger"
              >
                <Siren className="mr-1 inline h-3.5 w-3.5" /> Stop mission
              </button>
            ) : (
              <button
                type="button"
                ...
                className="rounded-xl bg-accent px-4 py-2.5 text-xs font-semibold text-bg disabled:opacity-40"
              >
                <ShieldCheck className="mr-1 inline h-3.5 w-3.5" />
                {busy ? "Starting…" : completed ? "Start update mission" : "Start SENTRY FIELD"}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[1.8fr_1fr_1fr]">
          <Info label="Exact tender" value={tender.title} detail={tender.reference_number} />
          <Info label="Primary requirement" value={req.label} detail={`${req.id} · expected ${req.expected_quantity}`} />
        ...
        <div className="mt-4 rounded-xl border border-accent/20 bg-accent/5 p-4">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.15em] text-accent">
            <Radar className="h-3.5 w-3.5" /> Auto-selected capabilities
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {capabilities.map((capability) => (
              <span key={capability} className="rounded-full border border-accent/25 bg-accent/10 px-3 py-1.5 text-[11px] font-semibold text-text">
        ...
        <div className="mt-4 rounded-xl border border-border bg-bg/20 p-4">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.14em] text-faint">
            <Camera className="h-3.5 w-3.5" /> Camera source
          </div>
          <div className="mt-2 grid gap-3 lg:grid-cols-[minmax(0,1fr)_260px]">
            <input
              value={camera}
              onChange={(event) => setCamera(event.target.value)}
              className="h-10 min-w-0 rounded-xl border border-border bg-bg px-3 text-xs text-text"
            />
            <div className="rounded-xl border border-border bg-bg px-3 py-2">
              <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-[.12em] text-faint">
                <span className="flex items-center gap-1"><SlidersHorizontal className="h-3.5 w-3.5" /> AI confidence</span>
                <span className="text-accent">{Math.round(confidence * 100)}%</span>
              </div>
              <input
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
            <div className="text-[10px] text-muted">
              Live capture is independent from AI inference. Lower confidence shows more candidates; higher confidence is stricter.
            </div>
            <label className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.12em] text-faint">
              Inference every
              <select
                value={everyNFrames}
                onChange={(event) => setEveryNFrames(Number(event.target.value))}
                className="h-9 rounded-lg border border-border bg-bg px-2 text-xs text-text"
              >
                {[1, 2, 3, 4].map((value) => <option key={value} value={value}>{value} frame{value > 1 ? "s" : ""}</option>)}
              </select>
            </label>
          </div>
          {error ? <div className="mt-3 rounded-xl border border-danger/20 bg-danger/10 p-3 text-xs text-danger">{error}</div> : null}
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div className="flex gap-1 rounded-lg bg-surface-2 p-1">
            <button type="button" onClick={() => setTab("live")} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "live" ? "bg-surface text-text" : "text-muted"}`}>
              Live camera
            </button>
            <button type="button" onClick={() => setTab("evidence")} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "evidence" ? "bg-surface text-text" : "text-muted"}`}>
              Evidence ({evidenceEvents.length})
            </button>
          </div>
          <div className="text-[10px] uppercase tracking-[.14em] text-faint">
            {uniqueDetections} unique detections · {evidenceEvents.length} evidence frames · {Math.round(status.fps || 0)} FPS
          </div>
        </div>
          ...
          <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="relative aspect-video overflow-hidden rounded-xl bg-black">
              {streamUrl ? (
                <img
                  key={streamUrl}
                  src={streamUrl}
                  alt="SENTRY FIELD live annotated camera"
                  className="h-full w-full object-contain"
                  onError={() => setError("FIELD live stream disconnected. Check the FIELD terminal and DroidCam source.")}
                />
              ) : (
                <div className="grid h-full place-items-center text-sm text-muted">
                  <div className="text-center">
                    <Camera className="mx-auto h-8 w-8 opacity-50" />
                    <div className="mt-2">Start SENTRY FIELD to open the live camera.</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-2 p-4">
              <div className="flex items-center justify-between">
                <div className="text-[10px] font-semibold uppercase tracking-[.15em] text-accent">AI detections</div>
                <div className="text-2xl font-semibold text-text">{uniqueDetections}</div>
              </div>
              <div className="mt-1 text-xs text-muted">Unique tracked findings, not per-frame repeats.</div>
              <div className="mt-4 space-y-2">
                {detectionEvents
                  .filter((event, index, all) => all.findIndex((item) => (item.track_id || "") === (event.track_id || "") && item.capability === event.capability) === index)
                  .slice(0, 8)
                  .map((event, index) => (
                    <div key={`${event.track_id || event.capability || "d"}-${index}`} className="rounded-lg border border-border p-3">
                      <div className="flex items-center gap-2 text-xs font-semibold text-text">
                        <CheckCircle2 className="h-3.5 w-3.5 text-accent" />
                        {LABELS[event.capability || ""] || event.capability || "Detection"}
                      </div>
                      <div className="mt-1 text-[10px] text-muted">
                        {event.confidence != null ? `confidence ${(event.confidence * 100).toFixed(0)}%` : ""}
                        {event.track_id ? ` · ${event.track_id}` : ""}
                      </div>
                    </div>
                  ))}
              </div>
            ...
            {evidenceEvents.length ? (
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {evidenceEvents.map((event, index) => (
                  <figure key={`${event.frame_url}-${index}`} className="overflow-hidden rounded-xl border border-border bg-bg/20">
                    <img
                      src={evidenceUrl(event.frame_url || "")}
                      alt={`${LABELS[event.capability || ""] || event.capability || "Detected"} evidence ${index + 1}`}
                      className="aspect-video w-full object-cover"
                      loading="lazy"
                      onError={(imageEvent) => { imageEvent.currentTarget.alt = "Evidence image unavailable"; }}
                    />
                    <figcaption className="p-3">
                      <div className="text-xs font-semibold text-text">{LABELS[event.capability || ""] || event.capability || "Detected evidence"}</div>
                      <div className="mt-1 text-[10px] text-muted">
                        {event.confidence != null ? `Confidence ${(event.confidence * 100).toFixed(0)}%` : "Evidence frame"}
                        {event.gps?.lat != null && event.gps.lon != null ? ` · GPS ${event.gps.lat.toFixed(5)}, ${event.gps.lon.toFixed(5)}` : ""}
                      </div>
                    </figcaption>
                  </figure>
                ))}
              ...
              <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted">
                <Camera className="mx-auto h-7 w-7 opacity-60" />
                <div className="mt-2">No detected evidence frame yet.</div>
                <div className="mt-1 text-xs">Move the camera over the target; one evidence frame is retained per tracked detection.</div>
              </div>
            )}
          </div>
        )}
      ...
      <section className="rounded-2xl border border-border bg-surface p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <div className="mt-1 text-sm font-semibold text-text">Append-only submission record</div>
            <div className="mt-1 text-xs text-muted">Previous versions stay immutable; deletion is disabled.</div>
          </div>
          {live ? (
            <button type="button" onClick={() => void submitVerification()} disabled={busy} className="rounded-xl bg-accent px-4 py-2.5 text-xs font-semibold text-bg disabled:opacity-40">
              {busy ? "Saving…" : "Complete Verification"}
            </button>
          ) : null}
        </div>
        ...
        <div className="mt-4 space-y-2">
          {verification?.history?.length ? (
            [...verification.history].reverse().map((item) => (
              <div key={item.id} className="flex items-center justify-between rounded-xl border border-border bg-bg/20 p-3">
                <div>
                  <div className="text-sm font-semibold text-text">
                    Version {item.version}
                    <span className="ml-2 text-[10px] uppercase tracking-wider text-accent">{item.status}</span>
                  </div>
                  <div className="mt-1 text-[10px] text-muted">
                    {item.submitted_at || ""} · {item.evidence_count} evidence · {item.gps_evidence_count} GPS
                  </div>
                </div>
                <span className="rounded-full border border-accent/20 px-2 py-1 text-[10px] font-semibold text-accent">immutable</span>
              </div>
            ))
          ) : (
            <div className="rounded-xl border border-border bg-bg/20 p-4 text-sm text-muted">No submitted verification yet.</div>
          )}
        </div>

        {!completed ? null : (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-accent/20 bg-accent/5 p-3 text-xs text-muted">
            <CheckCircle2 className="h-4 w-4 text-accent" />
            Update Verification creates a new version. Existing evidence remains auditable and cannot be deleted through SENTRY.
          </div>
        )}
      </section>
    </main>
