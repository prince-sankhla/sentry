        ...
        <div className="mt-4 rounded-xl border border-border bg-bg/20 p-3">
          ...
          <div className="mt-2 flex gap-2">
            <input
              ...
              className="h-10 min-w-0 flex-1 rounded-xl border border-border bg-bg px-3 text-xs text-text"
            />
            <input
              aria-label="AI inference cadence"
              type="number"
              min={1}
              max={10}
              value={everyNFrames}
              onChange={(event) => setEveryNFrames(Math.max(1, Math.min(10, Number(event.target.value) || 1)))}
              className="h-10 w-24 rounded-xl border border-border bg-bg px-3 text-xs text-text"
            />
          </div>
          <div className="mt-1.5 text-[10px] text-muted">
            Single DroidCam connection. FIELD capture stays independent from AI inference; cadence 1 means every fresh frame is eligible for inference.
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div className="flex gap-1 rounded-lg bg-surface-2 p-1">
            <button
              type="button"
              onClick={() => setTab("live")}
              className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "live" ? "bg-surface text-text" : "text-muted"}`}
            >
              Live camera
            </button>
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "evidence" ? "bg-surface text-text" : "text-muted"}`}
            >
              Evidence ({evidenceEvents.length})
            </button>
          </div>
          <div className="text-[10px] uppercase tracking-[.14em] text-faint">
            {status.findings} detections · {evidenceEvents.length} evidence frames
          </div>
        </div>
        ...
        {tab === "live" ? (
          <div className="grid gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_320px]">
            ...
            <div className="rounded-xl border border-border bg-surface-2 p-4">
              ...
              <div className="mt-2 text-3xl font-semibold text-text">{detectionEvents.length}</div>
              <div className="mt-1 text-xs text-muted">Detection events update independently from camera delivery.</div>
              <div className="mt-4 space-y-2">
                {detectionEvents.slice(0, 8).map((event, index) => (
                  <div key={`${event.track_id || "d"}-${index}`} className="rounded-lg border border-border p-3">
                ...
                {evidenceEvents.map((event, index) => (
                  <figure key={`${event.frame_url}-${index}`} className="overflow-hidden rounded-xl border border-border bg-bg/20">
                    ...
                    <figcaption className="p-3">
                      <div className="text-xs font-semibold text-text">
                        {LABELS[event.capability || ""] || event.capability || "Detected evidence"}
                      </div>
                      <div className="mt-1 text-[10px] text-muted">
                        {event.confidence != null ? `Confidence ${(event.confidence * 100).toFixed(0)}%` : "Evidence frame"}
                        {event.gps?.lat != null && event.gps.lon != null
                          ? ` · GPS ${event.gps.lat.toFixed(5)}, ${event.gps.lon.toFixed(5)}`
                          : ""}
                      </div>
                    </figcaption>
                  </figure>
