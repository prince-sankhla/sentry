...
type Tender = { id: string; tender_id?: string; reference_number: string; title: string; source_name: string; source_url?: string | null; source_verified_on?: string; contract_location?: string | null; category?: string | null; machine: string; demo_site?: string | null; requirements: Requirement[]; verification_notes?: string | null; auto_generated?: boolean };
type Gps = { status?: string; source?: string | null; lat?: number | null; lon?: number | null; accuracy_m?: number | null; captured_at?: number | null };
type EventItem = { type: string; capability?: string; observation?: string; confidence?: number; value?: string; track_id?: string | null; detector?: string; mission_id?: string | null; requirement_id?: string | null; tender_id?: string | null; observed_at?: number; frame_url?: string | null; gps?: Gps };
...
type Reanalysis = { status: string; mission_id: string; tender?: { reference_number: string; title: string }; summary: { expected_total: number; observed_total: number; gap_total: number; evidence_count: number; gps_evidence_count: number; observation_count: number }; discrepancies: { requirement_id: string; capability: string; label: string; expected: number; observed: number; gap: number; signal: string }[]; possible_explanations: string[]; next_checks: string[]; guardrail: string };
...
export function FieldMissionConsole({ tenderKey, requirementId }: { tenderKey?: string; requirementId?: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        let resolved: Tender | null = null;
        if (tenderKey) {
          const backend = await fetch(`${BACKEND}/api/investigations/tenders/${encodeURIComponent(tenderKey)}/field-verification`, { cache: "no-store" });
          if (backend.ok) resolved = await backend.json() as Tender;
        }
        if (!resolved && tenderKey) {
        ...
        if (alive) setError(reason instanceof Error ? reason.message : "FIELD verification plan could not be resolved");
      }
    };
    void load();
    return () => { alive = false; };
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const payload = await eventsResponse.json() as { events?: EventItem[] };
        if (alive) {
  ...
  async function startMission() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.detail || "FIELD mission authorisation failed");
      startGps(missionId);
  ...
  async function stopMission() {
    ...
    finally { setStreamUrl(null); stopGps(); setStatus((current) => ({ ...current, running: false, authorized: false })); }
  }
  ...
  return (
    <main className="mx-auto w-full max-w-[1900px] space-y-5">
      <section className="rounded-2xl border border-accent/20 bg-surface p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">SENTRY selected the inspection capabilities from the tender. FIELD records live observations with mobile GPS and returns the evidence to the investigation.</p>
          </div>
        ...
        <div className="mt-4 rounded-xl border border-accent/20 bg-accent/5 p-4"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.15em] text-accent"><Radar className="h-3.5 w-3.5" /> Auto-selected capabilities</div><div className="mt-3 flex flex-wrap gap-2">{capabilities.map((capability) => <span key={capability} className="rounded-full border border-accent/25 bg-accent/10 px-3 py-1.5 text-[11px] font-semibold text-text">{LABELS[capability] || capability}</span>)}</div><div className="mt-2 text-[11px] text-muted">No manual detector selection. All executable capabilities in this tender mission run together.</div></div>
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_150px_150px]"><label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.14em] text-faint">Camera source</span><input value={camera} onChange={(event) => setCamera(event.target.value)} disabled={missionLive} className="h-10 w-full rounded-xl border border-border bg-bg/40 px-3 text-xs text-text outline-none focus:border-accent/50 disabled:opacity-70" placeholder="DroidCam /video URL" /></label><label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.14em] text-faint">Confidence</span><input type="number" min="0.05" max="0.99" step="0.01" value={confidence} onChange={(event) => setConfidence(Number(event.target.value))} disabled={missionLive} className="h-10 w-full rounded-xl border border-border bg-bg/40 px-3 text-xs text-text outline-none disabled:opacity-70" /></label><label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.14em] text-faint">Frame cadence</span><input type="number" min="1" max="60" value={everyNFrames} onChange={(event) => setEveryNFrames(Math.max(1, Number(event.target.value)))} disabled={missionLive} className="h-10 w-full rounded-xl border border-border bg-bg/40 px-3 text-xs text-text outline-none disabled:opacity-70" /></label></div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6"><Metric label="State" value={missionLive ? "LIVE" : status.authorized ? "READY" : "IDLE"} /><Metric label="FPS" value={status.fps.toFixed(1)} icon={<Gauge className="h-4 w-4" />} /><Metric label="Inference" value={`${status.inference_ms.toFixed(0)} ms`} icon={<Gauge className="h-4 w-4" />} /><Metric label="Findings" value={String(status.findings)} icon={<Radar className="h-4 w-4" />} /><Metric label="Evidence" value={String(status.evidence)} icon={<FileText className="h-4 w-4" />} /><Metric label="GPS" value={gpsLive ? `${status.gps.lat!.toFixed(5)}, ${status.gps.lon!.toFixed(5)}` : "Waiting"} icon={<MapPin className="h-4 w-4" />} /></section>

      <section className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        <div className="flex flex-col gap-3 border-b border-border px-4 py-3 md:flex-row md:items-center md:justify-between"><div className="flex gap-1 rounded-lg bg-surface-2 p-1"><button type="button" onClick={() => setTab("live")} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "live" ? "bg-surface text-text" : "text-muted"}`}>Live camera</button><button type="button" onClick={() => setTab("evidence")} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${tab === "evidence" ? "bg-surface text-text" : "text-muted"}`}>Evidence ({evidenceEvents.length})</button></div><div className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[.14em] text-faint"><span>Mission {status.mission_id || "—"}</span>{status.machine_id ? <span>· {status.machine_id}</span> : null}</div></div>
        {tab === "live" ? <div className="p-3"><div className="relative min-h-[55vh] overflow-hidden rounded-xl bg-[#0b0e13]">{streamUrl ? <img key={streamUrl} src={streamUrl} alt="SENTRY FIELD live annotated camera" className="h-full min-h-[55vh] w-full object-contain" onLoad={() => setCameraError(null)} onError={() => setCameraError("Camera stream failed. Check the DroidCam /video URL, phone network reachability, and FIELD gateway terminal.")} /> : <div className="absolute inset-0 grid place-items-center text-center"><div><Camera className="mx-auto h-10 w-10 text-accent" /><div className="mt-3 text-sm font-semibold text-text">{status.last_error ? "Field stream stopped" : "SENTRY FIELD ready"}</div><div className="mt-1 max-w-md text-xs leading-5 text-muted">Enter your phone's DroidCam /video URL, then start the mission. The first frame is shown only after the gateway opens the camera successfully.</div></div>}<div className="absolute left-4 top-4 flex flex-wrap gap-2"><Badge tone={missionLive ? "success" : "muted"}>{missionLive ? "LIVE" : "IDLE"}</Badge>{status.last_detection ? <Badge tone="success">{LABELS[status.last_detection.type] || status.last_detection.type} · {Math.round(status.last_detection.confidence * 100)}%</Badge> : null}</div><div className="absolute bottom-4 left-4 flex flex-wrap gap-2"><Badge tone={gpsLive ? "success" : "muted">GPS {status.gps.status || "unavailable"}</Badge></div></div></div> : <div className="divide-y divide-border">{evidenceEvents.length ? evidenceEvents.slice(0, 100).map((event, index) => { const url = event.frame_url ? (event.frame_url.startsWith("http") ? event.frame_url : `${API}${event.frame_url}`) : null; return <div key={`${event.observed_at || 0}-${event.track_id || index}`} className="grid gap-4 p-4 md:grid-cols-[200px_1fr_auto] md:items-center"><div className="aspect-video overflow-hidden rounded-xl border border-border bg-surface-2">{url ? <a href={url} target="_blank" rel="noreferrer"><img src={url} alt="Captured SENTRY FIELD evidence" className="h-full w-full object-cover" /></a> : null}</div><div><div className="text-sm font-semibold text-text">{LABELS[event.capability || ""] || event.capability || "Field evidence"}</div><div className="mt-1 text-xs text-muted">{event.observation || event.value || `Detector ${event.detector || "vision"}`}</div><div className="mt-1 text-xs text-faint">{event.confidence != null ? `Confidence ${Math.round(event.confidence * 100)}%` : ""}{event.track_id ? ` · ${event.track_id}` : ""}</div>{event.gps?.lat != null && event.gps?.lon != null ? <div className="mt-1 text-xs text-faint">GPS {event.gps.lat.toFixed(6)}, {event.gps.lon.toFixed(6)}</div> : null}</div><Badge tone="success"><CheckCircle2 className="mr-1 inline h-3 w-3" />Captured</Badge></div>; }) : <div className="p-12 text-center text-sm text-muted">No evidence captured yet. Detected frames will appear here automatically.</div>}</div>}
      </section>
      <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={resetMission} disabled={busy} className="inline-flex items-center gap-2 rounded-xl border border-border bg-surface px-4 py-2.5 text-xs font-semibold text-text disabled:opacity-50"><RotateCcw className="h-3.5 w-3.5" /> Reset mission</button><button type="button" onClick={() => void completeAndSend()} disabled={!status.mission_id || busy || (!events.some((event) => event.type === "evidence") && !events.some((event) => event.type === "detection"))} className="inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-xs font-semibold text-bg disabled:opacity-40"><Send className="h-3.5 w-3.5" />{busy ? "Returning to SENTRY…" : "Complete verification → SENTRY"}</button></div>
    </main>
  );
}
