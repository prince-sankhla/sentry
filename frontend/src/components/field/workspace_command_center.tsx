...
const EMPTY: Status = {
  ...
  capabilities: [], confidence: 0.65, every_n_frames: 2, machine: null, demo_site: null, machine_id: null,
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string | number; onChange: (value: string) => void; type?: string }) {
  return (
    <label className="block">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="h-10 w-full rounded-xl border border-border bg-bg/40 px-3 text-xs text-text outline-none focus:border-accent/50" />
    </label>
  );
}
...
function matches(event: EventItem, capability: string) {
  const t = `${event.type} ${event.capability || ""}`.toLowerCase();
  if (capability === "pothole") return t.includes("pothole");
  if (capability === "road_crack") return t.includes("crack");
  if (capability === "streetlight") return t.includes("streetlight") || t.includes("utility pole");
  if (capability === "cctv_camera") return t.includes("cctv");
  if (capability === "signboard") return t.includes("signboard") || t.includes("road sign");
  if (capability === "drain") return t.includes("drain") || t.includes("manhole");
  if (capability === "solar_panel") return t.includes("solar panel");
  return false;
}

function evidenceUrl(frameUrl: string | null | undefined) {
  if (!frameUrl) return null;
  return frameUrl.startsWith("http") ? frameUrl : `${API}${frameUrl}`;
}
...
export function FieldWorkspaceCommandCenter() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/tenders`, { cache: "no-store" })
      .then((r) => { if (!r.ok) throw new Error("Tender catalog unavailable"); return r.json(); })
      .then((data) => {
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!r.ok) throw new Error("Gateway offline");
        const payload = await r.json() as Status;
        if (alive) { setStatus(payload); setOnline(true); }
  ...
  function selectTender(tender: Tender) {
    ...
    setSelected(Object.fromEntries(CAPS.map(([, k]) => [k, Boolean(tender.requirements.some((x) => x.capability === k))])));
    setError(null);
  }

  function chooseRequirement(r: Requirement) {
    setSelectedReq(r); setReq(r.id); setSelected((cur) => ({ ...cur, [r.capability]: true })); setError(null);
  }
  ...
  async function authorizeAndStart() {
    ...
    try {
      const dispatch = await fetch(`${API}/dispatch`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        tender_id: selectedTender.id, mission_id: mission.trim(), requirement_id: selectedReq.id, capability: selectedReq.capability,
        machine: selectedTender.machine, demo_site: selectedTender.demo_site,
      }) });
  ...
  async function stop() {
    try { await fetch(`${API}/stop`, { method: "POST" }); } finally { setStream(null); setStatus((s) => ({ ...s, running: false, authorized: false })); }
  }
  ...
  const observed = useMemo(() => {
    ...
    return new Set(status.recent_events.filter((e) => e.track_id && matches(e, selectedReq.capability)).map((e) => e.track_id)).size;
  ...
  const evidenceEvents = status.recent_events.filter((e) => e.type === "evidence" && e.frame_url);
    ...
    <main className="mx-auto w-full max-w-[1900px] space-y-5">
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.18em] text-accent"><Radar className="h-3.5 w-3.5" /> SENTRY FIELD / FRONTEND COMMAND CENTER</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => active ? void stop() : void authorizeAndStart()} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold text-text">{active ? "Stop inspection" : "Authorise & dispatch rover"}</button>
          </div>
        </div>

        <div className="mt-5 grid gap-3 xl:grid-cols-[1.3fr_1fr_1fr]">
        ...
        <div className="mt-3 rounded-xl border border-accent/20 bg-accent/5 p-3">
          <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.14em] text-accent"><Camera className="h-3.5 w-3.5" /> Camera source</div>
          <div className="mt-2 grid gap-2 md:grid-cols-[1fr_auto]">
            <input value={camera} onChange={(e) => setCamera(e.target.value)} placeholder="http://10.x.x.x:4747/video" aria-label="Camera URL" className="h-11 w-full rounded-xl border border-border bg-bg px-3 text-xs text-text outline-none focus:border-accent/60" />
            <button onClick={() => setCamera(camera.trim())} className="rounded-xl border border-border px-4 text-xs font-semibold text-text">Apply camera URL</button>
          </div>
          <div className="mt-1.5 text-[11px] text-muted">DroidCam / local rover camera URL. Example: http://10.91.92.137:4747/video</div>
        </div>

        {selectedTender && <div className="mt-4 grid gap-3 xl:grid-cols-[1.6fr_1.2fr_1fr]">
          <div className="rounded-xl border border-border bg-surface-2 p-4"><div className="text-[10px] uppercase tracking-[.14em] text-faint">Official tender</div><div className="mt-1 line-clamp-2 text-sm font-semibold text-text">{selectedTender.title}</div><div className="mt-1 text-xs text-muted">{selectedTender.contract_location} · {selectedTender.source_name}</div><a href={selectedTender.source_url} target="_blank" rel="noreferrer" className="mt-2 inline-block text-[11px] font-semibold text-accent">Open source record ↗</a></div>
          <div className="rounded-xl border border-accent/20 bg-accent/5 p-4"><div className="text-[10px] uppercase tracking-[.14em] text-accent">Inspection requirements</div><div className="mt-2 flex flex-wrap gap-2">{selectedTender.requirements.map((r) => <button key={r.id} onClick={() => chooseRequirement(r)} className={`rounded-lg border px-2.5 py-1.5 text-[11px] font-semibold ${selectedReq?.id === r.id ? "border-accent bg-accent/10 text-text" : "border-border text-muted"}`}>{r.label}</button>)}</div><div className="mt-2 text-xs text-muted">Machine: {selectedTender.machine} · Demo site: {selectedTender.demo_site}</div></div>
          <div className="rounded-xl border border-border bg-surface-2 p-4"><div className="text-[10px] uppercase tracking-[.14em] text-faint">Permission gate</div><div className="mt-2 flex items-center gap-2 text-sm font-semibold text-text">{status.authorized ? <CheckCircle2 className="h-4 w-4 text-accent" /> : <XCircle className="h-4 w-4 text-muted" />}{status.authorized ? "Dispatch authorised" : "Operator approval required"}</div><div className="mt-1 text-xs text-muted">Backend validates tender, requirement, capability and machine.</div></div>
        </div>}

        <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr_auto]">
          <div className="rounded-xl border border-border bg-surface-2 p-3"><div className="flex justify-between text-[10px] uppercase tracking-[.14em] text-faint"><span>Confidence threshold</span><b className="text-text">{Math.round(confidence * 100)}%</b></div><input type="range" min=".05" max=".99" step=".01" value={confidence} onChange={(e) => setConfidence(Number(e.target.value))} className="mt-3 w-full" /></div>
          <div className="rounded-xl border border-border bg-surface-2 p-3"><div className="flex justify-between text-[10px] uppercase tracking-[.14em] text-faint"><span>Detection frequency</span><b className="text-text">Every {freq} frames</b></div><input type="range" min="1" max="12" value={freq} onChange={(e) => setFreq(Number(e.target.value))} className="mt-3 w-full" /></div>
          <button onClick={() => void authorizeAndStart()} className="rounded-xl bg-accent px-5 text-xs font-semibold text-bg"><SlidersHorizontal className="mr-2 inline h-4 w-4" /> Apply & dispatch</button>
        </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[260px_minmax(0,1fr)_320px]">
        ...
        <div ref={viewer} className={`min-w-0 overflow-hidden rounded-2xl border border-border bg-surface shadow-sm ${full ? "bg-black p-3" : ""}`}>
          ...
          {tab === "evidence" && <div className="divide-y divide-border">{evidenceEvents.length === 0 ? <div className="p-8 text-center text-sm text-muted">No captured evidence yet.</div> : evidenceEvents.map((e, i) => <div key={`${e.track_id || "ev"}-${e.observed_at || i}`} className="grid gap-4 p-4 md:grid-cols-[180px_1fr]">{evidenceUrl(e.frame_url) ? <a href={evidenceUrl(e.frame_url) || "#"} target="_blank" rel="noreferrer"><img src={evidenceUrl(e.frame_url) || ""} alt={`${e.capability || e.type} evidence`} className="h-32 w-full rounded-xl border border-border object-cover" /></a> : <div className="h-32 rounded-xl border border-border bg-surface-2" />}<div><div className="text-sm font-semibold text-text">{e.capability || e.type}</div><div className="mt-1 text-xs text-muted">Confidence {Math.round((e.confidence || 0) * 100)}% · Detector {e.detector || "vision"}</div><div className="mt-1 text-xs text-faint">Mission {e.mission_id || "—"} · Requirement {e.requirement_id || "—"}</div><Badge tone="success">Captured evidence</Badge></div></div>)}</div>}
        ...
        <aside className="rounded-2xl border border-border bg-surface p-4 shadow-sm"><div className="text-[10px] font-semibold uppercase tracking-[.16em] text-faint">Mission state</div><div className="mt-3 space-y-3 text-xs"><div className="flex justify-between"><span className="text-muted">Camera</span><span className="max-w-[180px] truncate text-text">{camera}</span></div><div className="flex justify-between"><span className="text-muted">Requirement</span><span className="text-text">{selectedReq?.id || "—"}</span></div><div className="flex justify-between"><span className="text-muted">Machine</span><span className="text-text">{status.machine || selectedTender?.machine || "—"}</span></div><div className="flex justify-between"><span className="text-muted">GPS</span><span className="text-text">{status.gps.status}</span></div><div className="flex justify-between"><span className="text-muted">Battery</span><span className="text-text">{status.battery == null ? "—" : `${status.battery}%`}</span></div></div><div className="mt-5 rounded-xl border border-border bg-surface-2 p-3 text-xs leading-5 text-muted"><b className="text-text">Last detection</b><br />{status.last_detection ? `${status.last_detection.type} · ${Math.round(status.last_detection.confidence * 100)}%` : "No accepted detection"}</div><div className="mt-3 rounded-xl border border-accent/20 bg-accent/5 p-3 text-xs leading-5 text-muted"><b className="text-text">Evidence is preserved</b><br />Accepted detections are written by the local field gateway and surfaced here with their captured frame.</div></aside>
      </section>
    </main>
  );
}
