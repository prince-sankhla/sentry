...
const navGroups: {
  label: string;
  items: { href: string; icon: typeof LayoutDashboard; label: string; roles?: WorkspaceRole[] }[];
}[] = [
  {
    label: "Intelligence",
    items: [
...
function roleDefinition(role: WorkspaceRole) {
  return WORKSPACE_ROLES.find((item) => item.id === role) ?? WORKSPACE_ROLES[0];
}
  ...
  return (
    <div className="min-h-screen bg-bg text-text">
      ...
      <header className="fixed inset-x-0 top-0 z-40 flex h-16 items-center gap-3 border-b border-border bg-bg-2/80 px-4 backdrop-blur-xl">
        ...
        <button onClick={() => setPaletteOpen(true)} className="group ml-3 flex h-10 max-w-xl flex-1 items-center gap-2.5 rounded-xl border border-border bg-bg/50 px-3.5 text-left text-sm text-faint transition-all duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:border-border-strong hover:bg-surface/60" type="button">
          ...
          <kbd className="hidden items-center gap-0.5 rounded-md border border-border bg-bg-2 px-1.5 py-0.5 text-[10px] font-medium text-muted sm:flex">⌘K</kbd>
        </button>
        <div className="ml-auto flex items-center gap-2.5">
          <WorkspaceRoleSwitcher />
          <span className="hidden xl:block"><StatusChip label={currentRole.shortLabel} detail="Workspace" pulse /></span>
          <div className="mx-0.5 hidden h-5 w-px bg-border xl:block" />
          <Button variant="subtle" iconOnly aria-label="Notifications" className="relative" icon={<><Bell className="h-4 w-4" /><span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-danger ring-2 ring-bg-2" /></>} />
          <Link href="/profile" className="flex items-center gap-2 rounded-xl border border-border bg-surface/60 px-2 py-1.5 text-sm text-text transition-all duration-200 hover:border-border-strong md:pr-3"><span className="grid h-6 w-6 place-items-center rounded-lg bg-accent/12 text-accent"><UserCircle className="h-4 w-4" /></span><span className="hidden md:block">Analyst</span></Link>
        </div>
      </header>
