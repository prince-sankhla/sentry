...
import { Activity, Award, Bell, BookOpenCheck, Building2, ChevronsLeft, FileCheck2, FileText, Flag, FolderSearch, GitBranch, Globe2, Inbox, LayoutDashboard, Map as MapIcon, Menu, Radar, Search, Settings, Clock, UserCircle, Command, Zap, ClipboardCheck, BriefcaseBusiness, RadioTower, ShieldCheck } from "lucide-react";
...
export function AppShell({ children }: { children: ReactNode }) {
  const pathname = usePathname(); const [collapsed, setCollapsed] = useState(false); const [mobileOpen, setMobileOpen] = useState(false); const [paletteOpen, setPaletteOpen] = useState(false); const [role, setRole] = useState<WorkspaceRole>("public_investigator"); const isMarketing = pathname === "/";
  useEffect(() => { setRole(getStoredWorkspaceRole()); function onRoleChange(event: Event) { const next = (event as CustomEvent<WorkspaceRole>).detail; if (WORKSPACE_ROLES.some((item) => item.id === next)) setRole(next); } window.addEventListener("sentry:workspace-role", onRoleChange); return () => window.removeEventListener("sentry:workspace-role", onRoleChange); }, []);
  ...
  const width = collapsed ? 76 : 248; const currentRole = useMemo(() => roleDefinition(role), [role]); if (isMarketing) return <div className="min-h-screen">{children}</div>;
  return <div className="min-h-screen bg-bg text-text"><CommandPalette open={paletteOpen} onClose={() => setPaletteOpen(false)} /><header className="fixed inset-x-0 top-0 z-40 flex h-16 items-center gap-3 border-b border-border bg-bg-2/80 px-4 backdrop-blur-xl"><Button variant="ghost" size="sm" iconOnly className="lg:hidden" onClick={() => setMobileOpen((v) => !v)} aria-label="Toggle navigation" icon={<Menu className="h-5 w-5" />} /><Link href="/" className="group flex items-center gap-2.5 pl-1 pr-2" aria-label="SENTRY home"><Wordmark tagline="Evidence over assumptions" size="sm" /></Link><button onClick={() => setPaletteOpen(true)} className="group ml-3 flex h-10 max-w-xl flex-1 items-center gap-2.5 rounded-xl border border-border bg-bg/50 px-3.5 text-left text-sm text-faint transition-all duration-200 hover:border-border-strong hover:bg-surface/60" type="button"><Search className="h-4 w-4 text-muted group-hover:text-accent" /><span className="flex-1 truncate">Search companies, buyers, tenders, awards…</span><kbd className="hidden rounded-md border border-border bg-bg-2 px-1.5 py-0.5 text-[10px] font-medium text-muted sm:flex">⌘K</kbd></button><div className="ml-auto flex items-center gap-2.5"><WorkspaceRoleSwitcher /><span className="hidden xl:block"><StatusChip label={currentRole.shortLabel} detail="Workspace" pulse /></span><div className="mx-0.5 hidden h-5 w-px bg-border xl:block" /><Button variant="subtle" iconOnly aria-label="Notifications" className="relative" icon={<><Bell className="h-4 w-4" /><span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-danger ring-2 ring-bg-2" /></>} /><Link href="/profile" className="flex items-center gap-2 rounded-xl border border-border bg-surface/60 px-2 py-1.5 text-sm text-text transition-all hover:border-border-strong md:pr-3"><span className="grid h-6 w-6 place-items-center rounded-lg bg-accent/12 text-accent"><UserCircle className="h-4 w-4" /></span><span className="hidden md:block">Analyst</span></Link></div></header><motion.aside animate={{ width }} initial={false} transition={{ duration: 0.18, ease: EASE }} className={`fixed bottom-0 left-0 top-16 z-30 border-r border-border bg-bg-2/70 backdrop-blur-xl ${mobileOpen ? "block" : "hidden lg:block"}`}><div className="flex h-full flex-col overflow-y-auto px-3 py-5"><nav className="flex-1 space-y-6">{navGroups.map((group) => { const visibleItems = group.items.filter((item) => !item.roles || item.roles.includes(role)); if (!visibleItems.length) return null; return <div key={group.label}>{!collapsed && <div className="t-label px-2 pb-2.5">{group.label}</div>}<div className="space-y-0.5">{visibleItems.map((item) => { const Icon = item.icon; const active = pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href)); return <Link key={item.href} href={item.href} title={collapsed ? item.label : undefined} className={`group relative flex h-10 items-center gap-3 rounded-xl px-3 text-[13.5px] transition-colors duration-150 ${active ? "font-medium text-text" : "text-muted hover:bg-surface/50 hover:text-text"}`}>{active && <motion.span layoutId="nav-active" transition={SPRING} className="absolute inset-0 rounded-xl border border-border-strong/60 bg-surface-2" />}{active && <span className="absolute inset-y-2 left-0 z-10 w-0.5 rounded-full bg-accent" />}<Icon className={`relative z-10 h-[18px] w-[18px] shrink-0 ${active ? "text-accent" : "group-hover:text-text"}`} />{!collapsed && <span className="relative z-10 truncate">{item.label}</span>}</Link>; })}</div></div>; })}</nav>{!collapsed && <div className="mt-5 rounded-2xl border border-border bg-surface/60 p-4 elevate"><div className="flex items-center gap-2 text-[13.5px] font-semibold text-text"><span className="grid h-6 w-6 place-items-center rounded-lg border border-accent/25 bg-accent/[0.08] text-accent"><Zap className="h-3.5 w-3.5" /></span>New Investigation</div><p className="mt-2 text-[11.5px] leading-relaxed text-muted">{currentRole.description}</p><Button href="/investigations" variant="primary" size="sm" fullWidth className="mt-3.5" trailing={<Command className="h-3.5 w-3.5 opacity-70" />}>Start</Button></div>}<Button variant="ghost" size="sm" onClick={() => setCollapsed((v) => !v)} className="mt-3 hidden justify-start lg:flex" icon={<ChevronsLeft className={`h-4 w-4 transition-transform duration-200 ${collapsed ? "rotate-180" : ""}`} />}>{!collapsed && "Collapse"}</Button></div></motion.aside>{mobileOpen && <div className="fixed inset-0 top-16 z-20 bg-bg/60 backdrop-blur-sm lg:hidden" onClick={() => setMobileOpen(false)} />}<motion.main animate={{ paddingLeft: width }} initial={false} transition={{ duration: 0.18, ease: EASE }} className="min-h-screen pt-16 max-lg:!pl-0"><PageTransition>{children}</PageTransition></motion.main></div>;
}
